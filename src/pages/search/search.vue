		...
		<view class="search-keyword" @touchstart="blur">
			<scroll-view class="keyword-list-box" v-show="isShowKeywordList" scroll-y>
				...
				<view class="m-container">
					<template v-if="shopInfoType === 0">
						<shopInfo v-for="(item, key) in keywordList" :key="key" :info="item"></shopInfo>
					</template>
					<template v-else>
						<shopInfo v-for="(item, key) in keywordList" :key="key" :info="item" :origin='1'></shopInfo>
					</template>
					<view class="no_more_data">没有更多数据啦~</view>
				</view>
			</scroll-view>
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.searchData) {
				this.searchData = JSON.parse(options.searchData)
				if (options.type == 1) {
					this.shopInfoType = 1
				}
			}
		...
		methods: {
			...
			loadDefaultKeyword() {
				...
				this.defaultKeyword = "店铺名称 注册号 手机号码 用户名";
			},
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['旗舰版', '连锁版', '5A连锁'];
			},
			//监听输入
			inputChange(event) {
				//兼容引入组件时传入参数情况
				...
				this.startSearch()
				// this.keywordList = this.searchType(keyword)
				// this.keywordList = this.drawCorrelativeKeyword(jsonData.result, keyword);
				//  uni.request({
			...
			startSearch() {
				...
				let arr = []
				let oFilter = this.searchData.forEach(item => {
					if (this.isInputNumber(keyword)) { // 如果搜索的是数字
						if (item.orgPhone.toString().indexOf(keyword) >= 0) {
							arr.push(item)
						}
						if (item.reg.toString().indexOf(keyword) >= 0) {
							arr.push(item)
						}
					} else {
						if (item.productName.toString().indexOf(keyword) >= 0) {
							arr.push(item)
						}
						if (item.orgLoginName.toString().indexOf(keyword) >= 0) {
							arr.push(item)
						}
					}
					return;
